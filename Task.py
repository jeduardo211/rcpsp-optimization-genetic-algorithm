...
class Task:
    ...
    def add_sucessor(self, task: "Task"):
        self.sucessors = np.append(self.sucessors, task)
        task.predecessors = np.append(task.predecessors, self)

    def add_predecessors(self, tasks: np.ndarray["Task"]):
        for task in tasks:
            self.add_predecessor(task)

    def add_sucessors(self, tasks: np.ndarray["Task"]):
        for task in tasks:
            self.add_sucessor(task)

    def add_renewable_resource(self, resource: Resource, capacity: int):
