...
class SMFileParser:
    ...
    def parse_sm_file(file_path):
        ...
        nonrenewable_pattern = re.compile(
            r"^\s*-\s+nonrenewable\s*:\s*(\d+)\s*"
        )
        doubly_constrained_pattern = re.compile(
            r"^\s*-\s+doubly constrained\s*:\s*(\d+)\s*"
        )
        ...
        for line in lines:
            ...
            if in_resource_section:
                ...
                doubly_constrained_match = doubly_constrained_pattern.match(
                    line
                )
                if renewable_match:
                    ...
                    doubly_constrained_resources = int(
                        doubly_constrained_match.group(1)
                    )
            ...
            # Parse precedence relations
                ...
                if match:
                    ...
                    successors = (
                        list(map(int, successors.strip().split()))
                        if successors
                        else []
                    )
            ...
            # Parse requests and durations
                ...
                if match:
                    ...
                    resources_data = (
                        list(map(int, resources_data.strip().split()))
                        if resources_data
                        else []
                    )
                    requests_durations.append(
                        (int(jobnr), int(mode), int(duration), resources_data)
                    )
            ...
            # Parse resource availabilities
                ...
                if match:
                    resource_availabilities.append(
        ...
        precedence_df = pd.DataFrame(
            precedence_relations,
            columns=["jobnr", "modes", "successors_count", "successors"],
        )
        requests_df = pd.DataFrame(
            requests_durations,
            columns=["jobnr", "mode", "duration", "resources"],
        )
        availability_df = pd.DataFrame(
            resource_availabilities, columns=["R1", "R2", "R3", "R4"]
        )
    ...
    # Parse the file
    resource_df, project_info, precedence_df, requests_df, availability_df = (
        parse_sm_file(file_path)
    )
