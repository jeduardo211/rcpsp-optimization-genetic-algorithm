...
class Schedule:
    ...
    def remove_task(
        ...
        self.tasks = np.delete(
            self.tasks, np.where(self.tasks == Task)
        ).tolist()
    ...
    def add_renewable_resource(self, resource: Resource):
        self.renewable_resources = np.append(
            self.renewable_resources, resource
        )

    def add_renewable_resources(self, resources: np.ndarray[Resource]):
    ...
    def is_valid_precedence_relations_constraint(self) -> bool:
        for i in range(len(self.tasks)):
            ...
            for predecessor in task.predecessors:
                if (
                    predecessor not in subarray
                    and predecessor.name != self.dummySource.name
                    and predecessor.name != self.dummySink.name
                ):
                    return False
    ...
    def forward_recursion(self) -> bool:
        ...
        for task in self.tasks:
            if task == self.dummySource:
                ...
                earliest_start = np.max(
                    [
                        predecessor.earliest_finish
                        for predecessor in task.predecessors
                    ]
                )
    ...
    def backward_recursion(self) -> bool:
        ...
        for task in reversed(self.tasks):
            if task == self.dummySink:
                continue
            latest_finish = np.min(
                [sucessor.latest_start for sucessor in task.sucessors]
            )
    ...
    def adjust_time_intervals_to_resource_constraints(self) -> None:
        for resource in self.renewable_resources:
            ...
            for task in self.tasks:
                for i in range(task.earliest_start, task.earliest_finish):
                    if (
                        resources_per_time[i]
                        + task.renewable_resources[resource]
                        <= resource.per_period_availability
                    ):  # TODO: expandir para fazer para todos os recursos
                        resources_per_time[i] += task.renewable_resources[
                            resource
                        ]
                        continue
                    for time in range(
                        task.earliest_start, len(resources_per_time)
                    ):
                        if (
                            resources_per_time[time]
                            + task.renewable_resources[resource]
                            > resource.per_period_availability
                        ):
                            resources_per_time = np.append(
                                resources_per_time, 0
                            )
                            continue
                        else:
                            ...
                            for sucessor in task.sucessors:
                                ...
                                sucessor.earliest_finish = (
                                    sucessor.earliest_start + sucessor.duration
                                )
                            for i in range(
                                task.earliest_start, task.earliest_finish
                            ):
                                if (
                                    resources_per_time[i]
                                    + task.renewable_resources[resource]
        ...
        self.dummySink.earliest_finish = np.max(
            [task.earliest_finish for task in self.tasks]
        )
